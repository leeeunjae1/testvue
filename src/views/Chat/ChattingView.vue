...
export default {
  ...
  methods: {
    ...
    this.$axios.post('https://jejurang.site:8080/api/v1/chat/create', chatRoomData)
      .then(response => {
    ...
    async fetchChatRooms() {
      try {
        const response = await this.$axios.get('https://jejurang.site:8080/api/v1/chat/list');
    ...
    deleteChatRoom(roomId) {
      this.$axios.delete(`https://jejurang.site:8080/api/v1/chat/room/${roomId}`)
        .then(() => {
          alert("채팅방이 삭제되었습니다.");
    ...
    async fetchUserInfo() {
      try {
        const response = await this.$axios.get('https://jejurang.site:8080/api/v1/user/info/detail');
