...
export default {
  ...
  async mounted() {
    // 컴포넌트가 마운트 되었을 때 console.log로 recommendEventId를 출력합니다.
    console.log('EventChart 컴포넌트의 recommendEventId:', this.recommendEventId);
    try {
      const response = await this.$axios.get(`/api/recommendreply/event/reads/tag-counting/${this.recommendEventId}`);
      if (response.data) {
