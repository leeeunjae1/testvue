...
export default {
  ...
  async mounted() {
    // 컴포넌트가 마운트 되었을 때 console.log로 recommendFoodId를 출력합니다.
    console.log('FoodChart 컴포넌트의 recommendFoodId:', this.recommendFoodId);
    try {
      const response = await this.$axios.get(`/api/recommendreply/food/reads/tag-counting/${this.recommendFoodId}`);
      if (response.data) {
